...
def main():
    ...
    beta = 0.6
    r = 0.8
    null_scores, alt_scores = simulate_scores(n,beta,r,100,phi_test,2)

    fig, axs = plt.subplots(2)
    fig.suptitle('Simulated HC values')
    axs[0].hist(np.log(null_scores), bins = 100, range = [min(np.log(null_scores)),max(np.log(alt_scores))])
    axs[1].hist(np.log(alt_scores), bins = 100, range = [min(np.log(null_scores)),max(np.log(alt_scores))])
